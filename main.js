//variabel som lagrar datan från JSON-filen
let productData
...
//variabler för totalvärdet i varukorgen
let total = 0

//array till "Ta bort"-knapparna i varukorgen
let deleteBtn = []

//funktion som ritar ut produkterna i startsidan
function listProductData() {
    ...
    for (i = 0; i < productData.length; i++) {
        ...
        <div class="product-card" onload="productsLoaded()">
            ...
            <button class="btn-add-to-cart" id="btn${i}">Lägg till i kundvagnen</button>
        </div>
        </div>
        `
    }
    ...
    for (i = 0; i < productsToBuy.length; i++) {
        ...
        <div class="flexItem">
            <div id="produktVal">
                ...
                <button class="buttonDelete" id="deleteBtn${i}">Ta bort</button>
            </div>
        </div>
        `
...
//Lagrar datan i variabeln produktData. Sen kör funktion som ritar ut produkterna beroende på om man är på startsidan eller varukorgen
.then(products => {
...
//skapar funktion för knappar på startsida eller varukorg
.then(productsLoaded => {
    //om js inte hittar element med id main, vet den att man står i varukorg och kör följande kod
    if (document.getElementById("main") == null) {
        //ger idn till "ta bort"-knapparna
        for (i = 0; i < productsToBuy.length; i++) {
            deleteBtn[i] = document.getElementById("deleteBtn"+i)
        }

        //tar bort produkten från varukorgen (denna funkar inte helt ännu)
        for (i = 0; i < productsToBuy.length; i++) {
    ...
    //om js hittar elementet main vet den att man står på startsidan, då körs följande kod
        ...
        //vid klickkk: öka countern, skriv ut countern, lagra countern i localstorage
        //lägg produkten i en array som representerar produkten i varukorgen, lagrar arrayen i localstorage
